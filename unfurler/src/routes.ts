      ...
      async getData(params: string[]) {
        if (params?.length) {
          ...
          if (blockId.length !== 64) {
            blockId = await (
              await fetch(config.API.ESPLORA + `/block-height/${blockId}`, {
                agent: agentSelector,
              })
            ).text();
          }
  ...
  tx: {
    ...
    sip: {
      ...
      async getData(params: string[]) {
        if (params?.length) {
          let txid = params[0];
          const [transaction, times, cpfp, rbf, outspends]: any[] =
            await Promise.all([
              sipFetchJSON(config.API.ESPLORA + `/tx/${txid}`),
              sipFetchJSON(
                config.API.MEMPOOL + `/transaction-times?txId[]=${txid}`
              ),
          ...
          const features = transaction
            ? {
                segwit: transaction.vin.some(
                  (v) =>
                    v.prevout &&
                    ['v0_p2wsh', 'v0_p2wpkh'].includes(
                      v.prevout.scriptpubkey_type
                    )
                ),
                taproot: transaction.vin.some(
                  (v) => v.prevout && v.prevout.scriptpubkey_type === 'v1_p2tr'
                ),
                p2op: transaction.vin.some(
                  (v) => v.prevout && v.prevout.scriptpubkey_type === 'p2op'
                ),
...
export const networks = {
  ...
  liquid: {
    ...
    routes: {
      // only block, address & tx routes supported
      block: routes.block,
  ...
  bitb: {
    ...
    description:
      'BITB provides low-cost access to bitcoin through a professionally managed fund',
